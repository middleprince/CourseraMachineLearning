...
%compute ai and y
%     z_i+1 = a_i * theta_i'
%     a_i+1 = [ones(m,1), g(a_i+1)];   add the bias term to it
a_1 = [ones(m,1),X];
z_2 = a_1 * Theta1';
...
%the more effecient way to compute cost J(theta)
J = sum( sum(-y_recode.*log_a3 - (1-y_recode).*log_a3_minus));
...
% the for-loop to compute the regulation term,it's less effecient
...
% the meore effecient way to compute regularization term
regular_term = sum( sum(Theta1_temp.^2)) + sum( sum(Theta2_temp.^2));
